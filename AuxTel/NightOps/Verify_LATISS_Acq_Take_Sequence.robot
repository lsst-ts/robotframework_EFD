...
Load Camera Playlist
    [Tags]    execute
    ${result}=    Run Process    load_camera_playlist    at    ${playlist}
...
Execute AuxTel LATISS Acquire
    [Tags]    execute
    IF     "${playlist}" == "test"
        Skip    msg="The TEST condition only executes the latiss_take_sequence process. Skipping the latiss_acquire process."
    ELSE
...
Verify ATDome AzimuthInPosition
    [Tags]
    IF     "${playlist}" == "test"
        Skip    msg="The TEST condition skips the latiss_acquire process."
    ELSE
...
Verify ATCamera Acquire Image Sequence
    [Documentation]    Verify the ATCamera images are the correct type, with the correct exposure time.
    [Tags]    robot:continue-on-failure
    IF     "${playlist}" == "test"
        Skip    msg="The TEST condition skips the latiss_acquire process."
    ELSE
        ${cmd_df}=    Get Recent Samples    ATCamera    command_takeImages    ["expTime", "keyValueMap", "numImages", "shutter",]    ${acq_seq_length}    None
        ${evt_df}=    Get Recent Samples    ATCamera    logevent_startIntegration    ["additionalValues", "exposureTime", "imageName"]    ${acq_seq_length}    None
        Set Suite Variable    @{image_names}    ${evt_df.imageName.values}
        Verify Sequence    ATCamera    command_takeImages    expTime    ${acq_seq_length}    ${acq_exp_time}
        Verify Sequence    ATCamera    logevent_startIntegration    exposureTime    ${acq_seq_length}    ${acq_exp_time}
        FOR    ${i}    IN RANGE    ${acq_seq_length}
            ${evt_image_type}=    Fetch From Left    ${evt_df.iloc[${i}].additionalValues}    :
            Should Be Equal As Strings    ${evt_image_type}    ${acq_img_type_seq}[${i}]
            ${image_type_str}=    Fetch From Left    ${cmd_df.iloc[${i}].keyValueMap}    ,
            ${cmd_image_type}=    Fetch From Right    ${image_type_str}    :${SPACE}
            Should Be Equal As Strings    ${cmd_image_type}    ${acq_img_type_seq}[${i}]
...
Verify ATOODS Acquire ImageInOODS
    [Tags]
    IF     "${playlist}" == "test"
        Skip    msg="The TEST condition skips the latiss_acquire process."
    ELSE
        Wait Until Keyword Succeeds    60 sec    10 sec    Verify Image in OODS    ATOODS    ${image_names}[0][0]
        ${dataframe}=    Get Recent Samples    ATOODS    logevent_imageInOODS    ["camera", "description", "obsid",]    ${acq_seq_length}    None
        FOR    ${i}    IN RANGE    ${acq_seq_length}
...
Verify ATHeaderService Acquire LargeFileObjectAvailable
    [Tags]
    IF     "${playlist}" == "test"
        Skip    msg="The TEST condition skips the latiss_acquire process."
    ELSE
        ${dataframe}=    Get Recent Samples    ATHeaderService    logevent_largeFileObjectAvailable    ["id", "url",]    ${acq_seq_length}    None
        FOR    ${i}    IN RANGE    ${acq_seq_length}
...
Verify ATCamera Take Sequence Images
    [Documentation]    Verify the ATCamera images are the correct type, with the correct exposure time.
    [Tags]    robot:continue-on-failure
...
Verify ATOODS Take Sequence ImageInOODS
    [Tags]
    Wait Until Keyword Succeeds    60 sec    10 sec    Verify Image in OODS    ATOODS    ${image_names}[0][0]
...
Verify ATHeaderService Take Sequence LargeFileObjectAvailable
    [Tags]
...
Set Variables
    ...
    ELSE IF    "${playlist}" == "nominal"
        ...
        Set Suite Variable    @{acq_exp_time}    ${2}    ${2}
