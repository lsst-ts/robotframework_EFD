...
argfile=$HOME/trunk/robotframework_EFD/Jenkins_EFD_Tables_Test.list
...
echo "###  Test Suite list ###" >> $argfile
echo "#  Change the title of the test results" >> $argfile
echo "--name Jenkins_EFD_Tables" >> $argfile
echo "--output EFD_Tables" >> $argfile
echo "" >> $argfile
echo "# List of test suites" >> $argfile

# Find test suites and add them to the argument file.
ls -1 $HOME/trunk/robotframework_EFD/EFD_Tables |sed 's/^/rbtefd\/EFD_Tables\//' >> $argfile
